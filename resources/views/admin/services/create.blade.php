...
@section('title', 'Create API Service')
...
<div class="p-6 lg:p-8">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-jakarta">Create API Service</h1>
        <p class="mt-1 text-gray-500">Add a new API service to the platform.</p>
    </div>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm px-4 py-2.5 border">{{ old('description') }}</textarea>
            </div>

            <div>
                <label for="base_url" class="block text-sm font-medium text-gray-700 mb-1">Base URL</label>
                <input type="url" name="base_url" id="base_url" value="{{ old('base_url') }}" placeholder="https://api.example.com/v1"
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm px-4 py-2.5 border">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="rotation_strategy" id="rotation_strategy"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm px-4 py-2.5 border">
                        <option value="round_robin" {{ old('rotation_strategy') == 'round_robin' ? 'selected' : '' }}>Round Robin</option>
                        <option value="priority" {{ old('rotation_strategy') == 'priority' ? 'selected' : '' }}>Priority</option>
                        <option value="least_used" {{ old('rotation_strategy') == 'least_used' ? 'selected' : '' }}>Least Used</option>
                        <option value="weighted" {{ old('rotation_strategy') == 'weighted' ? 'selected' : '' }}>Weighted</option>
                        <option value="fill_rotate" {{ old('rotation_strategy') == 'fill_rotate' ? 'selected' : '' }}>Fill & Rotate</option>
                    </select>
                </div>

                <div>
                    <label for="credits_per_request" class="block text-sm font-medium text-gray-700 mb-1">Credits Per Request</label>
                    <input type="number" name="credits_per_request" id="credits_per_request" value="{{ old('credits_per_request', 1) }}" min="0" step="0.01"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm px-4 py-2.5 border">
                </div>

                <div>
                    <label for="rate_limit_per_minute" class="block text-sm font-medium text-gray-700 mb-1">Rate Limit / Minute</label>
                    <input type="number" name="rate_limit_per_minute" id="rate_limit_per_minute" value="{{ old('rate_limit_per_minute', 60) }}" min="1"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 text-sm px-4 py-2.5 border">
                </div>
            </div>
