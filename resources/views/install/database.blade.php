...
<div class="p-6 lg:p-8" x-data="{ loading: false }">
    ...
    <form method="POST" action="{{ route('install.save-database') }}" @submit="loading = true" class="space-y-4">
        @csrf
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Host</label>
                <input type="text" name="db_host" value="{{ old('db_host', '127.0.0.1') }}" required :disabled="loading"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none disabled:opacity-50">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Port</label>
                <input type="number" name="db_port" value="{{ old('db_port', '3306') }}" required :disabled="loading"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none disabled:opacity-50">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="db_name" value="{{ old('db_name', 'xapiverse_db') }}" required :disabled="loading"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none disabled:opacity-50">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="db_user" value="{{ old('db_user', 'root') }}" required :disabled="loading"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none disabled:opacity-50">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Password</label>
                <input type="password" name="db_password" value="{{ old('db_password') }}" :disabled="loading"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none disabled:opacity-50" placeholder="Empty if none">
            </div>
        </div>
